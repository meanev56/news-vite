  ...
  if (isLoading) {
    return (
      <div className="space-y-4">
        <h2 className="text-xl font-bold border-b-2 border-red-600 inline-block pb-1 mb-4">TOP NEWS</h2>
        ...
        {articles.map((article) => (
          <div key={article.id} className="border-b border-gray-200 pb-4">
            <Link to={`/article/${article.slug}`}>
              <h3 
                className="font-bold text-base mb-1 hover:text-news-accent transition-colors" 
